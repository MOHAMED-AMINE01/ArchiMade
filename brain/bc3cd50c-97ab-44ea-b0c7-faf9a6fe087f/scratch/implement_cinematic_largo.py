...
# --- 2. UPDATED ARCHIPROJECTDETAIL (ZOOM + STAGGER) ---
updated_detail = """
                            ...
                            {i === 0 && project.pathBefore ? (
                                <div className="h-[80vh] md:h-screen w-full relative cinematic-img">
                                    ...
                                    <img 
                                        ...
                                        style={{ clipPath: `inset(0 \${100 - (50)}% 0 0)` }}
                                        alt="After" 
                                    />
                                </div>
                            ) : (
...
# --- 3. UPDATED ARCHIGALLERY (LOGIC FLOW) ---
updated_gallery = """
function ArchiGallery() {
    ...
    const handleTransitionComplete = () => {
        ...
        // We keep isTransitioning true for a moment or handle fade out in Detail
...
# Apply all changes
content = re.sub(r'// --- PROJECT DETAIL OVERLAY.*?function ArchiGallery\(\) \{.*?\}', transition_component + "\\n" + updated_detail + "\\n" + updated_gallery, content, flags=re.DOTALL)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)

print("Implemented cinematic transition, zoom entry, and staggering text.")
